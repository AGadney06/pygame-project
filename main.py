...
class Game:
    ...
    def alien_setup(self,rows,cols,x_distance = 60 ,y_distance = 48,x_offset = 70, y_offset = 100):
        ...
        for row_index, row in enumerate(range(rows)): #Determines what row we are talking about
            for col_index, col in enumerate(range(cols)):
                ...
                if row_index == 0:
                    alien_sprite = Alien('invader1',x,y,alien_speed) #On row 0, print out this alien type
                elif 1 <= row_index <= 2:
                    alien_sprite = Alien('invader2',x,y,alien_speed) #On row 1 and row 2, print out this alien type
                else:
                    alien_sprite = Alien('invader3',x,y,alien_speed)
        ...
        if self.player.sprite.lasers:
            for laser in self.player.sprite.lasers:
                ...
                if pygame.sprite.spritecollide(laser,self.aliens,True):
                    laser.kill()
    def alien_speed_checker(self):
        if len(self.aliens) >= 48:
            self.alien_speed = 1
        elif len(self.aliens) >= 24:
            self.alien_speed = 1.5
        elif len(self.aliens) >= 12:
            self.alien_speed = 2
        elif len(self.aliens) >= 6:
    ...
    def run(self):
        ...
        self.player.update() #Updates what the player is doing
        self.player.draw(screen) #Draws the player to the screen
        self.blocks.draw(screen) #Draws blocks to the screen
        self.aliens.update(self.alien_direction,self.alien_speed) #Constantly updates the aliens to use the functions alien_direction and alien_speed
        self.aliens.draw(screen) #Draws the Aliens to the screen
        self.alien_location_checker() #Checks the location of the aliens
        self.alien_lasers.update() #Updates the alien lasers
        self.alien_lasers.draw(screen) #Draws Alien lasers to screen
        self.collision_check() #Checks for collision of everything
        self.alien_speed_checker() #Checks the alien's speed
